...
;;; Not open-coding cars and cdrs, since the calling conventions are slightly awkward.
...
;a-lmp-trail (0)
a-lmp-area (0)
a-lmp-mode (0)					;0001  means occurs check.
						;0101  means cyclic structures.
						;0201  means lexical closures.
...
lmp-utemplate-nd				;Here if we don't need to dereference.
   ...
   (ERROR-TABLE ARGTYP PROLOG-TEMPLATE M-A)
...
lmp-uconstruct					;X & construct
        ...
        (jump-data-type-not-equal pdl-top (a-constant (byte-value q-data-type dtp-list))
				  lmp-uconstruct-1)
	(call carcdr-no-sb)			;List & construct.
...
lmp-construct-list-rest
	(call carcdr-no-sb)
        ((pdl-push) m-t)
	(jump-equal-xct-next m-a a-v-nil lmp-construct-list-cdr)
       ((m-t) m-a)
	(call lmp-construct-t)
...
lmpu-list-list					;Here to unify two lists.
	 (call-if-bit-set (byte-field 1 1) m-i lmpu-unbind-later)
	 ((m-c) m-s)				;C gets first arg,
	 (call-xct-next carcdr-no-sb)		;A and T get its parts.
...
lmp-invoke
	...
	(jump-if-bit-clear (byte-field 1 2) m-i lmp-invoke-not-lexical)
	(call p3zero)
	((pdl-push) m-t)
	(dispatch (i-arg 0) d-lmp-mmjcall i-dont-chain)


lmp-invoke-not-lexical
	(call carcdr-no-sb)			;A := fctn, T := args.
	(dispatch-call (i-arg lmpoff-true) d-read-exit-vector i-dont-chain)
	((md) q-typed-pointer md)
	(jump-equal md a-a xtrue)		;Succeed if fctn is TRUE.
	(dispatch-call (i-arg lmpoff-false) d-read-exit-vector i-dont-chain)
	((md) q-typed-pointer md)
	(popj-equal md a-a)			;Fail if fctn is FALSE.
	((pdl-push) m-a)
	(jump-xct-next uaply)			;APPLY otherwise.
       ((pdl-push) m-t)
...
lmp-centry
	...
	(check-page-read)
	(dispatch transport md)
	(call-xct-next carcdr-no-sb)		;A := latest universe, T := defn.
       ((m-t) q-typed-pointer md)
...
lmp-xload
	 ((pdl-push) dpb m-zero (byte-field 5 23) a-a)
	 (jump xload-from-higher-context)
